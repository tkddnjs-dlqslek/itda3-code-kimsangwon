"""파인튜닝 rec(korean_PP-OCRv5_rec_ft.onnx)로 골드 1,000장 재실행. 한 줄마다 flush, 이어하기 지원."""
import os, sys, csv, time
sys.path.insert(0, os.path.join(os.path.dirname(__file__), "..", "src"))
import ocr
ocr._REC_FILES["ko"] = "korean_PP-OCRv5_rec_ft.onnx"   # 엔진 캐시 전에 교체
from pipeline import predict_one, list_images
ids = {l.strip() for l in open("labels/gold_ids.txt", encoding="utf-8") if l.strip()}
out = "labels/auto_gold_v5_ft.csv"
done = set()
