...
WAIT_FOR = "labels/auto_gold_v5_ft.csv"
while os.path.exists(WAIT_FOR) and sum(1 for _ in open(WAIT_FOR, encoding="utf-8")) - 1 < 1000:
    time.sleep(60)
import ocr
ocr._REC_FILES["ch"] = "korean_PP-OCRv5_rec_ft.onnx"   # 숫자 재판독만 파인튜닝 모델로
from pipeline import predict_one, list_images
ids = {l.strip() for l in open("labels/gold_ids.txt", encoding="utf-8") if l.strip()}
out = "labels/auto_gold_v5_ftdigit.csv"
done = set()
